...
/*
 * Controlador para los personalizados de plan de cirugia
 */
	...
	if( isset($_SESSION['usuario_idUsuario']) and $_SESSION['usuario_idUsuario'] != '' ){ 
		...
		if(isset($_POST['envioNuevoPCI']) and $_POST['envioNuevoPCI'] == "envio" ){
			
			//se reciben las variables
			$titulo		= $_POST['tituloPersonalizadoPCI'];
			$texto		= trim($_POST['planCirugia']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
				...
				$idUsuario = $_SESSION['usuario_idUsuario'];
			 	...
			 	$guardarPersonalizado  = $objetoPersonalizados->guardarPersonalizadoPlanCirugia($titulo, $texto, $idUsuario);
		        
		        $_SESSION['mensaje']   = $ok;	

				//se redirecciona nuevamente a personalizados 
				 header('Location: '.Config::ruta().'personalizados/planCirugia/' );	
		...
		if(isset($_POST['idPersonalizadoPCI']) and $_POST['idPersonalizadoPCI'] != "" ){
					
						//se reciben las variables
			$idPersonalizado	= $_POST['idPersonalizadoPCI'];			
			$titulo				= $_POST['editarTituloPersonalizadoPCI'];
			$texto				= trim($_POST['editarPlanCirugia']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
			 	...
			 	$editarPersonalizado  = $objetoPersonalizados->editarPersonalizadoPlanCirugia($idPersonalizado, $titulo, $texto);
 	...
 	//se consulta el total de los personalizados existentes
		$TotalPersonalizados = $objetoPersonalizados->tatalPersonalizadosPCI();	
		...
		$listadoPersonalizados = $objetoPersonalizados->listarPersonalizadosPCI($idUsuario, $paginaActual,$records_per_page);//se llama al metodo para listar los personalizados segun los limites de la consulta
