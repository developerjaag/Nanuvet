 ...
 */
	...
	if( isset($_SESSION['usuario_idUsuario']) and $_SESSION['usuario_idUsuario'] != '' ){ 
		...
		if(isset($_POST['envioNuevoMCI']) and $_POST['envioNuevoMCI'] == "envio" ){
			
			//se reciben las variables
			$titulo		= $_POST['tituloPersonalizadoMCI'];
			$texto		= trim($_POST['motivoCirugia']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
				...
				$idUsuario = $_SESSION['usuario_idUsuario'];
			 	...
			 	$guardarPersonalizado  = $objetoPersonalizados->guardarPersonalizadoMotivoCirugia($titulo, $texto, $idUsuario);
		        
		        $_SESSION['mensaje']   = $ok;	

				//se redirecciona nuevamente a personalizados 
				 header('Location: '.Config::ruta().'personalizados/motivoCirugia/' );	
		...
		if(isset($_POST['idPersonalizadoMCI']) and $_POST['idPersonalizadoMCI'] != "" ){
					
						//se reciben las variables
			$idPersonalizado	= $_POST['idPersonalizadoMCI'];			
			$titulo				= $_POST['editarTituloPersonalizadoMCI'];
			$texto				= trim($_POST['editarMotivoCirugia']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
			 	...
			 	$editarPersonalizado  = $objetoPersonalizados->editarPersonalizadoMotivoCirugia($idPersonalizado, $titulo, $texto);
 	...
 	//se consulta el total de los personalizados existentes
		$TotalPersonalizados = $objetoPersonalizados->tatalPersonalizadosMCI();	
		...
		$listadoPersonalizados = $objetoPersonalizados->listarPersonalizadosMCI($idUsuario, $paginaActual,$records_per_page);//se llama al metodo para listar los personalizados segun los limites de la consulta
