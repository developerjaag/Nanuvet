...
/*
 * Controlador para los personalizados de observaciones de formulacion
 */
	...
	if( isset($_SESSION['usuario_idUsuario']) and $_SESSION['usuario_idUsuario'] != '' ){ 
		...
		if(isset($_POST['envioNuevoOF']) and $_POST['envioNuevoOF'] == "envio" ){
			
			//se reciben las variables
			$titulo		= $_POST['tituloPersonalizadoOF'];
			$texto		= trim($_POST['observacionesFormula']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
				...
				$idUsuario = $_SESSION['usuario_idUsuario'];
			 	...
			 	$guardarPersonalizado  = $objetoPersonalizados->guardarPersonalizadoObservacionesFormula($titulo, $texto, $idUsuario);
		        
		        $_SESSION['mensaje']   = $ok;	

				//se redirecciona nuevamente a personalizados 
				 header('Location: '.Config::ruta().'personalizados/observacionesFormula/' );	
		...
		if(isset($_POST['idPersonalizadoOF']) and $_POST['idPersonalizadoOF'] != "" ){
					
						//se reciben las variables
			$idPersonalizado	= $_POST['idPersonalizadoOF'];			
			$titulo				= $_POST['editarTituloPersonalizadoOF'];
			$texto				= trim($_POST['editarObservacionesFormula']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
			 	...
			 	$editarPersonalizado  = $objetoPersonalizados->editarPersonalizadoObservacionesFormula($idPersonalizado, $titulo, $texto);
 	...
 	//se consulta el total de los personalizados existentes
		$TotalPersonalizados = $objetoPersonalizados->tatalPersonalizadosOF();	
		...
		$listadoPersonalizados = $objetoPersonalizados->listarPersonalizadosOF($idUsuario, $paginaActual,$records_per_page);//se llama al metodo para listar los personalizados segun los limites de la consulta
