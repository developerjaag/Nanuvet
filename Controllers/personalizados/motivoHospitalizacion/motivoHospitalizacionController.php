...
/*
 * Controlador para los personalizados de motivo de hospitalizacion
 */
	...
	if( isset($_SESSION['usuario_idUsuario']) and $_SESSION['usuario_idUsuario'] != '' ){ 
		...
		if(isset($_POST['envioNuevoMH']) and $_POST['envioNuevoMH'] == "envio" ){
			
			//se reciben las variables
			$titulo		= $_POST['tituloPersonalizadoMH'];
			$texto		= trim($_POST['motivoHospitalizacion']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
				...
				$idUsuario = $_SESSION['usuario_idUsuario'];
			 	...
			 	$guardarPersonalizado  = $objetoPersonalizados->guardarPersonalizadoMotivoHospitalizacion($titulo, $texto, $idUsuario);
		        
		        $_SESSION['mensaje']   = $ok;	

				//se redirecciona nuevamente a personalizados 
				 header('Location: '.Config::ruta().'personalizados/motivoHospitalizacion/' );	
		...
		if(isset($_POST['idPersonalizadoMH']) and $_POST['idPersonalizadoMH'] != "" ){
					
						//se reciben las variables
			$idPersonalizado	= $_POST['idPersonalizadoMH'];			
			$titulo				= $_POST['editarTituloPersonalizadoMH'];
			$texto				= trim($_POST['editarMotivoHospitalizacion']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
			 	...
			 	$editarPersonalizado  = $objetoPersonalizados->editarPersonalizadoMotivoHospitalizacion($idPersonalizado, $titulo, $texto);
 	...
 	//se consulta el total de los personalizados existentes
		$TotalPersonalizados = $objetoPersonalizados->tatalPersonalizadosMH();	
		...
		$listadoPersonalizados = $objetoPersonalizados->listarPersonalizadosMH($idUsuario, $paginaActual,$records_per_page);//se llama al metodo para listar los personalizados segun los limites de la consulta
