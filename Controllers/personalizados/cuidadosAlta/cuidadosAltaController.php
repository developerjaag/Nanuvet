 ...
 */
	...
	if( isset($_SESSION['usuario_idUsuario']) and $_SESSION['usuario_idUsuario'] != '' ){ 
		...
		if(isset($_POST['envioNuevoCA']) and $_POST['envioNuevoCA'] == "envio" ){
			
			//se reciben las variables
			$titulo		= $_POST['tituloPersonalizadoCA'];
			$texto		= trim($_POST['cuidadosAlta']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
				...
				$idUsuario = $_SESSION['usuario_idUsuario'];
			 	...
			 	$guardarPersonalizado  = $objetoPersonalizados->guardarPersonalizadoCuidadosAlta($titulo, $texto, $idUsuario);
		        
		        $_SESSION['mensaje']   = $ok;	

				//se redirecciona nuevamente a personalizados 
				 header('Location: '.Config::ruta().'personalizados/cuidadosAlta/' );	
		...
		if(isset($_POST['idPersonalizadoCA']) and $_POST['idPersonalizadoCA'] != "" ){
					
						//se reciben las variables
			$idPersonalizado	= $_POST['idPersonalizadoCA'];			
			$titulo				= $_POST['editarTituloPersonalizadoCA'];
			$texto				= trim($_POST['editarCuidadosAlta']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
			 	...
			 	$editarPersonalizado  = $objetoPersonalizados->editarPersonalizadoCuidadosAlta($idPersonalizado, $titulo, $texto);
 	...
 	//se consulta el total de los personalizados existentes
		$TotalPersonalizados = $objetoPersonalizados->tatalPersonalizadosCA();	
		...
		$listadoPersonalizados = $objetoPersonalizados->listarPersonalizadosCA($idUsuario, $paginaActual,$records_per_page);//se llama al metodo para listar los personalizados segun los limites de la consulta
