...
/*
 * Controlador para los personalizados de observaciones de examen fisico
 */
	...
	if( isset($_SESSION['usuario_idUsuario']) and $_SESSION['usuario_idUsuario'] != '' ){ 
		...
		if(isset($_POST['envioNuevoOEF']) and $_POST['envioNuevoOEF'] == "envio" ){
			
			//se reciben las variables
			$titulo		= $_POST['tituloPersonalizadoOEF'];
			$texto		= trim($_POST['observacionesExamenFisico']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
				...
				$idUsuario = $_SESSION['usuario_idUsuario'];
			 	...
			 	$guardarPersonalizado  = $objetoPersonalizados->guardarPersonalizadoObservacionesExamenFisico($titulo, $texto, $idUsuario);
		        
		        $_SESSION['mensaje']   = $ok;	

				//se redirecciona nuevamente a personalizados 
				 header('Location: '.Config::ruta().'personalizados/observacionesExamenFisico/' );	
		...
		if(isset($_POST['idPersonalizadoOEF']) and $_POST['idPersonalizadoOEF'] != "" ){
					
						//se reciben las variables
			$idPersonalizado	= $_POST['idPersonalizadoOEF'];			
			$titulo				= $_POST['editarTituloPersonalizadoOEF'];
			$texto				= trim($_POST['editarObservacionesExamenFisico']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
			 	...
			 	$editarPersonalizado  = $objetoPersonalizados->editarPersonalizadoObservacionesExamenFisico($idPersonalizado, $titulo, $texto);
 	...
 	//se consulta el total de los personalizados existentes
		$TotalPersonalizados = $objetoPersonalizados->tatalPersonalizadosOEF();	
		...
		$listadoPersonalizados = $objetoPersonalizados->listarPersonalizadosOEF($idUsuario, $paginaActual,$records_per_page);//se llama al metodo para listar los personalizados segun los limites de la consulta
