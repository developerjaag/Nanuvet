 ...
 */
	...
	if( isset($_SESSION['usuario_idUsuario']) and $_SESSION['usuario_idUsuario'] != '' ){ 
		...
		if(isset($_POST['envioNuevoPC']) and $_POST['envioNuevoPC'] == "envio" ){
			
			//se reciben las variables
			$titulo		= $_POST['tituloPersonalizadoPC'];
			$texto		= trim($_POST['planConsulta']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
				...
				$idUsuario = $_SESSION['usuario_idUsuario'];
			 	...
			 	$guardarPersonalizado  = $objetoPersonalizados->guardarPersonalizadoPlanConsulta($titulo, $texto, $idUsuario);
		        
		        $_SESSION['mensaje']   = $ok;	

				//se redirecciona nuevamente a personalizados 
				 header('Location: '.Config::ruta().'personalizados/planConsulta/' );	
		...
		if(isset($_POST['idPersonalizadoPC']) and $_POST['idPersonalizadoPC'] != "" ){
					
						//se reciben las variables
			$idPersonalizado	= $_POST['idPersonalizadoPC'];			
			$titulo				= $_POST['editarTituloPersonalizadoPC'];
			$texto				= trim($_POST['editarPlanConsulta']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
			 	...
			 	$editarPersonalizado  = $objetoPersonalizados->editarPersonalizadoPlanConsulta($idPersonalizado, $titulo, $texto);
 	...
 	//se consulta el total de los personalizados existentes
		$TotalPersonalizados = $objetoPersonalizados->tatalPersonalizadosPC();	
		...
		$listadoPersonalizados = $objetoPersonalizados->listarPersonalizadosPC($idUsuario, $paginaActual,$records_per_page);//se llama al metodo para listar los personalizados segun los limites de la consulta
