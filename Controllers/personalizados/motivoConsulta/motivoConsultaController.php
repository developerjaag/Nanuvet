 ...
 */
	...
	if( isset($_SESSION['usuario_idUsuario']) and $_SESSION['usuario_idUsuario'] != '' ){ 
		...
		if(isset($_POST['envioNuevoMC']) and $_POST['envioNuevoMC'] == "envio" ){
			
			//se reciben las variables
			$titulo		= $_POST['tituloPersonalizadoMC'];
			$texto		= trim($_POST['motivoConsulta']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
				...
				$idUsuario = $_SESSION['usuario_idUsuario'];
			 	...
			 	$guardarPersonalizado  = $objetoPersonalizados->guardarPersonalizadoMotivoConsulta($titulo, $texto, $idUsuario);
		        
		        $_SESSION['mensaje']   = $ok;	

				//se redirecciona nuevamente a personalizados 
				 header('Location: '.Config::ruta().'personalizados/motivoConsulta/' );	
		...
		if(isset($_POST['idPersonalizadoMC']) and $_POST['idPersonalizadoMC'] != "" ){
					
						//se reciben las variables
			$idPersonalizado	= $_POST['idPersonalizadoMC'];			
			$titulo				= $_POST['editarTituloPersonalizadoMC'];
			$texto				= trim($_POST['editarMotivoConsulta']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
			 	...
			 	$editarPersonalizado  = $objetoPersonalizados->editarPersonalizadoMotivoConsulta($idPersonalizado, $titulo, $texto);
 	...
 	//se consulta el total de los personalizados existentes
		$TotalPersonalizados = $objetoPersonalizados->tatalPersonalizadosMC();	
		...
		$listadoPersonalizados = $objetoPersonalizados->listarPersonalizadosMC($idUsuario, $paginaActual,$records_per_page);//se llama al metodo para listar los personalizados segun los limites de la consulta
