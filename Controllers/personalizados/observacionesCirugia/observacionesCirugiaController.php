...
/*
 * Controlador para los personalizados de motivo de cirugia
 */
	...
	if( isset($_SESSION['usuario_idUsuario']) and $_SESSION['usuario_idUsuario'] != '' ){ 
		...
		if(isset($_POST['envioNuevoOCI']) and $_POST['envioNuevoOCI'] == "envio" ){
			
			//se reciben las variables
			$titulo		= $_POST['tituloPersonalizadoOCI'];
			$texto		= trim($_POST['observacionesCirugia']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
				...
				$idUsuario = $_SESSION['usuario_idUsuario'];
			 	...
			 	$guardarPersonalizado  = $objetoPersonalizados->guardarPersonalizadoObservacionesCirugia($titulo, $texto, $idUsuario);
		        
		        $_SESSION['mensaje']   = $ok;	

				//se redirecciona nuevamente a personalizados 
				 header('Location: '.Config::ruta().'personalizados/observacionesCirugia/' );	
		...
		if(isset($_POST['idPersonalizadoOCI']) and $_POST['idPersonalizadoOCI'] != "" ){
					
						//se reciben las variables
			$idPersonalizado	= $_POST['idPersonalizadoOCI'];			
			$titulo				= $_POST['editarTituloPersonalizadoOCI'];
			$texto				= trim($_POST['editarObservacionesCirugia']);
		    ...
		    if(($titulo == "") or ($texto == "")){
		     ...
		     if($sw == 0){
			 	...
			 	$editarPersonalizado  = $objetoPersonalizados->editarPersonalizadoObservacionesCirugia($idPersonalizado, $titulo, $texto);
 	...
 	//se cirugia el total de los personalizados existentes
		$TotalPersonalizados = $objetoPersonalizados->tatalPersonalizadosOCI();	
		...
		$listadoPersonalizados = $objetoPersonalizados->listarPersonalizadosOCI($idUsuario, $paginaActual,$records_per_page);//se llama al metodo para listar los personalizados segun los limites de la cirugia
