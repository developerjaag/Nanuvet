...
/*
 * Sibcontrolador para el reporte de hospitalizaicones
 * */
...
if(isset($_POST['envioFormFiltros']) and  $_POST['envioFormFiltros'] == 'enviar'){
	...
	//se redirecciona nuevamente a reportes hospitalizaciones
     header('Location: '.Config::ruta().'reportes/hospitalizaciones/' );	
...
if( isset($_SESSION['filtro_post']) AND  $_SESSION['filtro_post'] == "Si"){
				...
				$listadoHospitalizaciones = $objetoReportes->hospitalizaciones_aplicarFiltrosHospitalizaciones($fechaInicial, $fechaFinal, $usuarios, $idPropietario, $paciente);
			
				$listadoHospitalizacionesExportar = $listadoHospitalizaciones;
				
				$totalRegistros = $objetoReportes->hospitalizaciones_TotalAplicarFiltrosHospitalizaciones($fechaInicial, $fechaFinal, $usuarios, $idPropietario, $paciente);

	
		}else{
  
			 //se consulta el total de las hospitalizaciones existentes
				$TotalHospitalizaciones = $objetoReportes->hospitalizaciones_tatalHospitalizaciones();	
				...
				$listadoHospitalizaciones = $objetoReportes->hospitalizaciones_listarHospitalizaciones($paginaActual,$records_per_page);//se llama al metodo para listar las hospitalizaciones segun los limites de la consulta
				...
				//para el contenido a exportar, son todas las hospitalizaciones	
				$listadoHospitalizacionesExportar = $objetoReportes->hospitalizaciones_listarHospitalizaciones('0',$TotalHospitalizaciones);	
			//fin paginación
			
			$totalRegistros = $TotalHospitalizaciones;
