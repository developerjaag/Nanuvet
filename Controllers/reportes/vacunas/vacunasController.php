...
/*
 * Sibcontrolador para el reporte de las vacunas
 * */
...
if(isset($_POST['envioFormFiltros']) and  $_POST['envioFormFiltros'] == 'enviar'){
	...
	$_SESSION['filtro_4']			= $_POST['idVacuna'];
	...
	//se redirecciona nuevamente a reportes vacunas
     header('Location: '.Config::ruta().'reportes/vacunas/' );	
...
if( isset($_SESSION['filtro_post']) AND $_SESSION['filtro_post'] == "Si"){
				...
				$listadoVacunas = $objetoReportes->vacunas_aplicarFiltrosVacunas($fechaInicial, $fechaFinal, $fechaInicialProximo, $fechaFinalProximo, $idVacuna, $idPropietario, $paciente);
			
				$listadoVacunasExportar = $listadoVacunas;
				
				$totalRegistros = $objetoReportes->vacunas_TotalAplicarFiltrosVacunas($fechaInicial, $fechaFinal, $fechaInicialProximo, $fechaFinalProximo, $idVacuna, $idPropietario, $paciente);

	
		}else{
  
			 //se consulta el total de las vacunas existentes
				$TotalVacunas = $objetoReportes->vacunas_tatalVacunas();	
				...
				$listadoVacunas = $objetoReportes->vacunas_listarVacunas($paginaActual,$records_per_page);//se llama al metodo para listar las vacunas segun los limites de la consulta
				...
				//para el contenido a exportar, son todos las vacunas	
				$listadoVacunasExportar = $objetoReportes->vacunas_listarVacunas('0',$TotalVacunas);	
			//fin paginación
			
			$totalRegistros = $TotalVacunas;
